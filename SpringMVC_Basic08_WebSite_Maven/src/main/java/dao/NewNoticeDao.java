...
@Repository // 컨테이너 안에서 생성하고 싶지 않다면
NewNoticeDao 에  Annotation  붙이고 싶다면 ... xml 맨 상단에 <component-scan element가 있어야 한다
*/
...
public class NewNoticeDao implements NoticeDao {
	...
	/*
	 * 일반적인 자동 매핑 (권장) String sql="SELECT * FROM NOTICES WHERE SEQ="+seq;
	 * try{}catch(Exception e) {} 처리 권장 return this.jdbctemplate.queryForObject(sql,
	 * new BeanPropertyRowMapper<Notice>(Notice.class));
	 */
	...
	public int update(Notice notice) throws ClassNotFoundException, SQLException {
		...
		return this.jdbctemplate.update(sql, notice.getTitle(), notice.getContent(), notice.getFileSrc(),
				notice.getFileSrc2(), notice.getSeq());
	}
