...
<%--              (${pageContext.request.userPrincipal.name})로그아웃</a></li> <!-- ${pageContext.request.userPrincipal.name} 현재 나의 아이디 --> --%>
<%--           </c:if> --%>
               <se:authorize access="!hasRole('ROLE_USER')">
                  <li><a href="${pageContext.request.contextPath}/joinus/login.htm">로그인</a></li>
               </se:authorize>
               <se:authentication property="name" var="loginuser"/> <!-- name 값이 가지는 값을  Loginuser에 넣어서 나중에 쓸 수 있음, ${pageContext.request.userPrincipal.name} 대체-->
               <se:authorize access="hasAnyRole('ROLE_ADMIN', 'ROLE_USER')">
                  <li><a href="${pageContext.request.contextPath}/logout">(${pageContext.request.userPrincipal.name})로그아웃</a></li>
               </se:authorize>
<%--          <li><a href="${pageContext.request.contextPath}/joinus/login.htm">로그인</a></li> 기존 태그 설정--%>
         	  <se:authorize access="!hasRole('ROLE_USER')">
	         	  <li><a href="${pageContext.request.contextPath}/joinus/join.htm">회원가입</a></li>
         	  </se:authorize>
         	  <se:authorize access="hasAnyRole('ROLE_ADMIN', 'ROLE_USER')">
	         	  <li><a href="${pageContext.request.contextPath}/joinus/memberConfirm.htm">회원정보수정</a></li>
         	  </se:authorize>
      </ul>
